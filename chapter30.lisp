...
(defun codegen-html (ops pretty)
  (let ((*pretty* pretty))
    `(prognou ,@(generate-code (optimize-static-output ops)) nil)))
...
(defmethod indent ((pp html-pretty-printer))
  (when *pretty*
    (incf (indention (printer pp)) (tab-width pp))))

(defmethod unindent ((pp html-pretty-printer))
  (when *pretty*
    (decf (indention (printer pp)) (tab-width pp))))
